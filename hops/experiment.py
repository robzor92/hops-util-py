...
def launch(map_fun, args_dict=None, name='no-name', local_logdir=False, versioned_resources=None, description=None):
    ...
    *args_dict* is specified.
    ...
    try:
        ...
        experiment_json = None
        if args_dict:
            experiment_json = experiment_utils._populate_experiment(name, 'launcher', 'EXPERIMENT', json.dumps(args_dict), versioned_path, description, app_id, None)
        else:
            experiment_json = experiment_utils._populate_experiment(name, 'launcher', 'EXPERIMENT', None, versioned_path, description, app_id, None)
...
def random_search(map_fun, boundary_dict, direction='max', samples=10, name='no-name', local_logdir=False, versioned_resources=None, description=None, optimization_key=None):
    ...
    *Parallel Experiment*
    ...
    try:
        ...
        experiment_json = experiment_utils._populate_experiment(name, 'random_search', 'PARALLEL_EXPERIMENTS', json.dumps(boundary_dict), versioned_path, description, app_id, direction)
...
def differential_evolution(objective_function, boundary_dict, direction = 'max', generations=4, population=6, mutation=0.5, crossover=0.7, cleanup_generations=False, name='no-name', local_logdir=False, versioned_resources=None, description=None, optimization_key=None):
    ...
    *Parallel Experiment*
    ...
    try:
        ...
        experiment_json = experiment_utils._populate_experiment(name, 'differential_evolution', 'PARALLEL_EXPERIMENTS', json.dumps(boundary_dict), versioned_path, description, app_id, direction)
