...
def _handle_return(retval, hdfs_exec_logdir, optimization_key):
    ...
    if not optimization_key and type(retval) is dict and len(retval.keys()) > 1:
        ...
        raise Exception('Specified optimization key {} not in returned dict, when returning multiple values in a dict the optimization_key argument must be set.'.format(optimization_key))
